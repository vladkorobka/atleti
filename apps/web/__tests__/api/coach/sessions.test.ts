...
// Фіксований майбутній робочий день (UTC wall-clock) у межах графіку
const DAY = '2030-06-17'
const at = (t: string) => `${DAY}T${t}:00.000Z`
...
beforeEach(async () => {
  const { Session, CoachBlock } = await import('@atleti/db')
...
describe('POST /api/coach/sessions — конфлікти', () => {
  ...
  it('перетин зі скасованим заняттям → 201 (баг подвійного запису)', async () => {
